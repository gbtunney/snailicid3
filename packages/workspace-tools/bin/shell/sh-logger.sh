...
BOLD='\033[1m'; UNDERLINE='\033[4m'; REVERSE='\033[7m'; RESET='\033[0m'
GRAY="$BBLACK"; GREY="$BBLACK"  # aliases
...
hrule()      { rule "${1:-=-}" "${2:-auto}"; }
spacer()     { local n="${1:-1}"; for _ in $(seq 1 "$n"); do echo ""; done; }

header()     { spacer; echo -e "${BOLD}${BWHITE}$*${RESET}"; rule "=" auto; }
subheader()  { echo -e "${BOLD}${CYAN}$*${RESET}"; rule "-" auto; }
section()    { spacer; echo -e "${BOLD}${BYELLOW}── $* ──${RESET}"; }
