## API Report File for "@snailicid3/workspace"

> Do not edit this file. It is a report generated by [API Extractor](https://api-extractor.com/).

```ts

import { CommandResult } from '@snailicid3/node-utils';
import { EnvironmentDefinition } from '@snailicid3/node-utils';
import { RunCommandOptions } from '@snailicid3/node-utils';
import { StringClassifiers } from '@snailicid3/node-utils';
import { z } from 'zod';

// @public
export const BASE_WORKSPACE_SCOPES: readonly ["root"];

// @public
export type BaseRelation = 'ahead' | 'behind' | 'diverged' | 'equal' | 'unknown';

// @public
export type BaseSyncStatus = 'ahead' | 'behind' | 'diverged' | 'in-sync' | 'unknown';

// @public (undocumented)
export type BranchActionKind =
/** Only when genuinely impossible (detached HEAD, publish off base). */
'block'
/** Create the target branch from base. */
| 'create'
/** Already on the right branch; keep going. */
| 'proceed'
/** Relink: switch to the already-existing target branch. */
| 'switch';

// @public (undocumented)
export type BranchDecision = {
    action: BranchActionKind;
    carriesDirtyChanges: boolean;
    offerUpdateWithBase: boolean;
    reason: string;
    targetBranch: string;
    warnings: Array<string>;
};

// @public
export type BranchOperation = 'continue' | 'publish' | 'start';

// @public
export type BranchPrefix = 'changeset' | 'release';

// @public
export type BranchState = {
    baseBranch: string;
    baseSync: BaseSyncStatus;
    currentBranch: string;
    currentKind: CurrentBranchKind;
    onTarget: boolean;
    targetBranch: string;
    targetExistsLocal: boolean;
    targetExistsRemote: boolean;
    workingTree: WorkingTreeStatus;
};

// @public (undocumented)
export function checkProtectedBranch(repoRoot: string, operation: 'commit' | 'push', environment?: NodeJS.ProcessEnv): void;

// @public (undocumented)
export function checkStagedFilenames(repoRoot: string): void;

// @public
export const classifyBranchKind: (branch: string, baseBranch: string) => CurrentBranchKind;

// @public
export const createBranchFromBase: (repoRoot: string, branch: string, startPoint: string, options?: {
    inherit?: boolean;
}) => void;

// @public
export const createPullRequestPlan: (input: PullRequestPlanInput) => PullRequestPlan;

// @public
export function createReleasePlan(input: CreateReleasePlanInput): ReleasePlan;

// @public
export type CreateReleasePlanInput = z.input<typeof createReleasePlanInputSchema>;

// @public
export const createReleasePlanInputSchema: z.ZodObject<{
    execution: z.ZodOptional<z.ZodObject<{
        operation: z.ZodLiteral<"observe">;
    }, z.core.$strict>>;
    packages: z.ZodArray<z.ZodObject<{
        access: z.ZodOptional<z.ZodEnum<{
            public: "public";
            restricted: "restricted";
        }>>;
        doctor: z.ZodObject<{
            artifact: z.ZodEnum<{
                unknown: "unknown";
                invalid: "invalid";
                valid: "valid";
            }>;
            dependencyClosure: z.ZodEnum<{
                unknown: "unknown";
                valid: "valid";
                blocked: "blocked";
            }>;
        }, z.core.$strict>;
        gitTag: z.ZodDiscriminatedUnion<[z.ZodObject<{
            selected: z.ZodLiteral<false>;
        }, z.core.$strict>, z.ZodObject<{
            name: z.ZodString;
            selected: z.ZodLiteral<true>;
        }, z.core.$strict>], "selected">;
        intent: z.ZodDiscriminatedUnion<[z.ZodObject<{
            source: z.ZodLiteral<"none">;
        }, z.core.$strict>, z.ZodObject<{
            bump: z.ZodEnum<{
                major: "major";
                minor: "minor";
                patch: "patch";
            }>;
            reason: z.ZodString;
            source: z.ZodLiteral<"changesets">;
        }, z.core.$strict>, z.ZodObject<{
            bump: z.ZodEnum<{
                major: "major";
                minor: "minor";
                patch: "patch";
            }>;
            reason: z.ZodString;
            source: z.ZodLiteral<"explicit">;
        }, z.core.$strict>], "source">;
        name: z.ZodString;
        policy: z.ZodDiscriminatedUnion<[z.ZodObject<{
            channel: z.ZodString;
            decision: z.ZodLiteral<"selected">;
            reason: z.ZodString;
        }, z.core.$strict>, z.ZodObject<{
            decision: z.ZodLiteral<"held">;
            reason: z.ZodString;
        }, z.core.$strict>], "decision">;
        private: z.ZodBoolean;
        registry: z.ZodObject<{
            distTags: z.ZodRecord<z.ZodString, z.ZodString>;
            registryUrl: z.ZodNullable<z.ZodURL>;
            state: z.ZodEnum<{
                exists: "exists";
                missing: "missing";
                unknown_auth: "unknown_auth";
                unknown_network: "unknown_network";
                unknown_registry: "unknown_registry";
            }>;
        }, z.core.$strict>;
        version: z.ZodString;
        versionState: z.ZodDiscriminatedUnion<[z.ZodObject<{
            state: z.ZodLiteral<"current">;
        }, z.core.$strict>, z.ZodObject<{
            intendedVersion: z.ZodString;
            state: z.ZodLiteral<"pending">;
        }, z.core.$strict>], "state">;
    }, z.core.$strict>>;
}, z.core.$strict>;

// @public
export function createReleasePreparePlan(input: CreateReleasePreparePlanInput): ReleasePreparePlan;

// @public (undocumented)
export type CreateReleasePreparePlanInput = z.input<typeof createReleasePreparePlanInputSchema>;

// @public (undocumented)
export const createReleasePreparePlanInputSchema: z.ZodObject<{
    baseBranch: z.ZodString;
    plan: z.ZodObject<{
        execution: z.ZodObject<{
            operation: z.ZodLiteral<"observe">;
        }, z.core.$strict>;
        packages: z.ZodArray<z.ZodObject<{
            access: z.ZodOptional<z.ZodEnum<{
                public: "public";
                restricted: "restricted";
            }>>;
            doctor: z.ZodObject<{
                artifact: z.ZodEnum<{
                    unknown: "unknown";
                    invalid: "invalid";
                    valid: "valid";
                }>;
                dependencyClosure: z.ZodEnum<{
                    unknown: "unknown";
                    blocked: "blocked";
                    valid: "valid";
                }>;
            }, z.core.$strict>;
            gitTag: z.ZodDiscriminatedUnion<[z.ZodObject<{
                selected: z.ZodLiteral<false>;
            }, z.core.$strict>, z.ZodObject<{
                name: z.ZodString;
                selected: z.ZodLiteral<true>;
            }, z.core.$strict>], "selected">;
            intent: z.ZodDiscriminatedUnion<[z.ZodObject<{
                source: z.ZodLiteral<"none">;
            }, z.core.$strict>, z.ZodObject<{
                bump: z.ZodEnum<{
                    major: "major";
                    minor: "minor";
                    patch: "patch";
                }>;
                reason: z.ZodString;
                source: z.ZodLiteral<"changesets">;
            }, z.core.$strict>, z.ZodObject<{
                bump: z.ZodEnum<{
                    major: "major";
                    minor: "minor";
                    patch: "patch";
                }>;
                reason: z.ZodString;
                source: z.ZodLiteral<"explicit">;
            }, z.core.$strict>], "source">;
            name: z.ZodString;
            policy: z.ZodDiscriminatedUnion<[z.ZodObject<{
                channel: z.ZodString;
                decision: z.ZodLiteral<"selected">;
                reason: z.ZodString;
            }, z.core.$strict>, z.ZodObject<{
                decision: z.ZodLiteral<"held">;
                reason: z.ZodString;
            }, z.core.$strict>], "decision">;
            private: z.ZodBoolean;
            registry: z.ZodObject<{
                distTags: z.ZodRecord<z.ZodString, z.ZodString>;
                registryUrl: z.ZodNullable<z.ZodURL>;
                state: z.ZodEnum<{
                    exists: "exists";
                    missing: "missing";
                    unknown_auth: "unknown_auth";
                    unknown_network: "unknown_network";
                    unknown_registry: "unknown_registry";
                }>;
            }, z.core.$strict>;
            version: z.ZodString;
            versionState: z.ZodDiscriminatedUnion<[z.ZodObject<{
                state: z.ZodLiteral<"current">;
            }, z.core.$strict>, z.ZodObject<{
                intendedVersion: z.ZodString;
                state: z.ZodLiteral<"pending">;
            }, z.core.$strict>], "state">;
            availableNextOperations: z.ZodArray<z.ZodEnum<{
                prepare: "prepare";
                observe: "observe";
                publish: "publish";
                tag: "tag";
            }>>;
            status: z.ZodEnum<{
                unknown_registry: "unknown_registry";
                blocked_dependency_closure: "blocked_dependency_closure";
                blocked_invalid_artifact: "blocked_invalid_artifact";
                blocked_unknown_artifact: "blocked_unknown_artifact";
                blocked_unknown_dependency_closure: "blocked_unknown_dependency_closure";
                pending_eligible: "pending_eligible";
                pending_held: "pending_held";
                private_unpublishable: "private_unpublishable";
                published: "published";
            }>;
        }, z.core.$strict>>;
        schemaVersion: z.ZodLiteral<1>;
        summary: z.ZodObject<{
            blocked: z.ZodNumber;
            eligible: z.ZodNumber;
            held: z.ZodNumber;
            packages: z.ZodNumber;
            private: z.ZodNumber;
            published: z.ZodNumber;
            unknown: z.ZodNumber;
        }, z.core.$strict>;
    }, z.core.$strict>;
    selection: z.ZodArray<z.ZodString>;
    slug: z.ZodString;
    workingTree: z.ZodOptional<z.ZodEnum<{
        unknown: "unknown";
        clean: "clean";
        dirty: "dirty";
    }>>;
}, z.core.$strict>;

// @public
export function createReleasePublishPlan(input: CreateReleasePublishPlanInput): ReleasePublishPlan;

// @public (undocumented)
export type CreateReleasePublishPlanInput = z.input<typeof createReleasePublishPlanInputSchema>;

// @public (undocumented)
export const createReleasePublishPlanInputSchema: z.ZodObject<{
    candidates: z.ZodArray<z.ZodObject<{
        artifact: z.ZodObject<{
            integrity: z.ZodString;
            name: z.ZodString;
            tarball: z.ZodString;
            version: z.ZodString;
        }, z.core.$strict>;
        doctor: z.ZodObject<{
            artifact: z.ZodEnum<{
                unknown: "unknown";
                invalid: "invalid";
                valid: "valid";
            }>;
            closure: z.ZodDiscriminatedUnion<[z.ZodObject<{
                edges: z.ZodArray<z.ZodDiscriminatedUnion<[z.ZodObject<{
                    name: z.ZodString;
                    range: z.ZodString;
                    resolution: z.ZodLiteral<"available_in_registry">;
                    satisfiedBy: z.ZodString;
                }, z.core.$strict>, z.ZodObject<{
                    name: z.ZodString;
                    range: z.ZodString;
                    resolution: z.ZodLiteral<"included_in_cohort">;
                }, z.core.$strict>, z.ZodObject<{
                    name: z.ZodString;
                    resolution: z.ZodLiteral<"embedded_not_exposed">;
                }, z.core.$strict>, z.ZodObject<{
                    name: z.ZodString;
                    range: z.ZodString;
                    resolution: z.ZodLiteral<"unavailable">;
                }, z.core.$strict>, z.ZodObject<{
                    name: z.ZodString;
                    resolution: z.ZodLiteral<"unknown">;
                }, z.core.$strict>], "resolution">>;
                state: z.ZodLiteral<"valid">;
            }, z.core.$strict>, z.ZodObject<{
                edges: z.ZodArray<z.ZodDiscriminatedUnion<[z.ZodObject<{
                    name: z.ZodString;
                    range: z.ZodString;
                    resolution: z.ZodLiteral<"available_in_registry">;
                    satisfiedBy: z.ZodString;
                }, z.core.$strict>, z.ZodObject<{
                    name: z.ZodString;
                    range: z.ZodString;
                    resolution: z.ZodLiteral<"included_in_cohort">;
                }, z.core.$strict>, z.ZodObject<{
                    name: z.ZodString;
                    resolution: z.ZodLiteral<"embedded_not_exposed">;
                }, z.core.$strict>, z.ZodObject<{
                    name: z.ZodString;
                    range: z.ZodString;
                    resolution: z.ZodLiteral<"unavailable">;
                }, z.core.$strict>, z.ZodObject<{
                    name: z.ZodString;
                    resolution: z.ZodLiteral<"unknown">;
                }, z.core.$strict>], "resolution">>;
                state: z.ZodLiteral<"blocked">;
            }, z.core.$strict>, z.ZodObject<{
                state: z.ZodLiteral<"unknown">;
            }, z.core.$strict>], "state">;
        }, z.core.$strict>;
        name: z.ZodString;
    }, z.core.$strict>>;
    channel: z.ZodString;
    plan: z.ZodObject<{
        execution: z.ZodObject<{
            operation: z.ZodLiteral<"observe">;
        }, z.core.$strict>;
        packages: z.ZodArray<z.ZodObject<{
            access: z.ZodOptional<z.ZodEnum<{
                public: "public";
                restricted: "restricted";
            }>>;
            doctor: z.ZodObject<{
                artifact: z.ZodEnum<{
                    unknown: "unknown";
                    invalid: "invalid";
                    valid: "valid";
                }>;
                dependencyClosure: z.ZodEnum<{
                    unknown: "unknown";
                    valid: "valid";
                    blocked: "blocked";
                }>;
            }, z.core.$strict>;
            gitTag: z.ZodDiscriminatedUnion<[z.ZodObject<{
                selected: z.ZodLiteral<false>;
            }, z.core.$strict>, z.ZodObject<{
                name: z.ZodString;
                selected: z.ZodLiteral<true>;
            }, z.core.$strict>], "selected">;
            intent: z.ZodDiscriminatedUnion<[z.ZodObject<{
                source: z.ZodLiteral<"none">;
            }, z.core.$strict>, z.ZodObject<{
                bump: z.ZodEnum<{
                    major: "major";
                    minor: "minor";
                    patch: "patch";
                }>;
                reason: z.ZodString;
                source: z.ZodLiteral<"changesets">;
            }, z.core.$strict>, z.ZodObject<{
                bump: z.ZodEnum<{
                    major: "major";
                    minor: "minor";
                    patch: "patch";
                }>;
                reason: z.ZodString;
                source: z.ZodLiteral<"explicit">;
            }, z.core.$strict>], "source">;
            name: z.ZodString;
            policy: z.ZodDiscriminatedUnion<[z.ZodObject<{
                channel: z.ZodString;
                decision: z.ZodLiteral<"selected">;
                reason: z.ZodString;
            }, z.core.$strict>, z.ZodObject<{
                decision: z.ZodLiteral<"held">;
                reason: z.ZodString;
            }, z.core.$strict>], "decision">;
            private: z.ZodBoolean;
            registry: z.ZodObject<{
                distTags: z.ZodRecord<z.ZodString, z.ZodString>;
                registryUrl: z.ZodNullable<z.ZodURL>;
                state: z.ZodEnum<{
                    unknown_auth: "unknown_auth";
                    unknown_network: "unknown_network";
                    unknown_registry: "unknown_registry";
                    missing: "missing";
                    exists: "exists";
                }>;
            }, z.core.$strict>;
            version: z.ZodString;
            versionState: z.ZodDiscriminatedUnion<[z.ZodObject<{
                state: z.ZodLiteral<"current">;
            }, z.core.$strict>, z.ZodObject<{
                intendedVersion: z.ZodString;
                state: z.ZodLiteral<"pending">;
            }, z.core.$strict>], "state">;
            availableNextOperations: z.ZodArray<z.ZodEnum<{
                publish: "publish";
                observe: "observe";
                prepare: "prepare";
                tag: "tag";
            }>>;
            status: z.ZodEnum<{
                unknown_registry: "unknown_registry";
                blocked_dependency_closure: "blocked_dependency_closure";
                blocked_invalid_artifact: "blocked_invalid_artifact";
                blocked_unknown_artifact: "blocked_unknown_artifact";
                blocked_unknown_dependency_closure: "blocked_unknown_dependency_closure";
                pending_eligible: "pending_eligible";
                pending_held: "pending_held";
                private_unpublishable: "private_unpublishable";
                published: "published";
            }>;
        }, z.core.$strict>>;
        schemaVersion: z.ZodLiteral<1>;
        summary: z.ZodObject<{
            blocked: z.ZodNumber;
            eligible: z.ZodNumber;
            held: z.ZodNumber;
            packages: z.ZodNumber;
            private: z.ZodNumber;
            published: z.ZodNumber;
            unknown: z.ZodNumber;
        }, z.core.$strict>;
    }, z.core.$strict>;
    selection: z.ZodArray<z.ZodString>;
}, z.core.$strict>;

// @public
export function createReleaseTagPlan(input: CreateReleaseTagPlanInput): ReleaseTagPlan;

// @public (undocumented)
export type CreateReleaseTagPlanInput = z.input<typeof createReleaseTagPlanInputSchema>;

// @public (undocumented)
export const createReleaseTagPlanInputSchema: z.ZodObject<{
    existingTags: z.ZodNullable<z.ZodArray<z.ZodString>>;
    plan: z.ZodObject<{
        execution: z.ZodObject<{
            operation: z.ZodLiteral<"observe">;
        }, z.core.$strict>;
        packages: z.ZodArray<z.ZodObject<{
            access: z.ZodOptional<z.ZodEnum<{
                public: "public";
                restricted: "restricted";
            }>>;
            doctor: z.ZodObject<{
                artifact: z.ZodEnum<{
                    unknown: "unknown";
                    invalid: "invalid";
                    valid: "valid";
                }>;
                dependencyClosure: z.ZodEnum<{
                    unknown: "unknown";
                    blocked: "blocked";
                    valid: "valid";
                }>;
            }, z.core.$strict>;
            gitTag: z.ZodDiscriminatedUnion<[z.ZodObject<{
                selected: z.ZodLiteral<false>;
            }, z.core.$strict>, z.ZodObject<{
                name: z.ZodString;
                selected: z.ZodLiteral<true>;
            }, z.core.$strict>], "selected">;
            intent: z.ZodDiscriminatedUnion<[z.ZodObject<{
                source: z.ZodLiteral<"none">;
            }, z.core.$strict>, z.ZodObject<{
                bump: z.ZodEnum<{
                    major: "major";
                    minor: "minor";
                    patch: "patch";
                }>;
                reason: z.ZodString;
                source: z.ZodLiteral<"changesets">;
            }, z.core.$strict>, z.ZodObject<{
                bump: z.ZodEnum<{
                    major: "major";
                    minor: "minor";
                    patch: "patch";
                }>;
                reason: z.ZodString;
                source: z.ZodLiteral<"explicit">;
            }, z.core.$strict>], "source">;
            name: z.ZodString;
            policy: z.ZodDiscriminatedUnion<[z.ZodObject<{
                channel: z.ZodString;
                decision: z.ZodLiteral<"selected">;
                reason: z.ZodString;
            }, z.core.$strict>, z.ZodObject<{
                decision: z.ZodLiteral<"held">;
                reason: z.ZodString;
            }, z.core.$strict>], "decision">;
            private: z.ZodBoolean;
            registry: z.ZodObject<{
                distTags: z.ZodRecord<z.ZodString, z.ZodString>;
                registryUrl: z.ZodNullable<z.ZodURL>;
                state: z.ZodEnum<{
                    exists: "exists";
                    missing: "missing";
                    unknown_auth: "unknown_auth";
                    unknown_network: "unknown_network";
                    unknown_registry: "unknown_registry";
                }>;
            }, z.core.$strict>;
            version: z.ZodString;
            versionState: z.ZodDiscriminatedUnion<[z.ZodObject<{
                state: z.ZodLiteral<"current">;
            }, z.core.$strict>, z.ZodObject<{
                intendedVersion: z.ZodString;
                state: z.ZodLiteral<"pending">;
            }, z.core.$strict>], "state">;
            availableNextOperations: z.ZodArray<z.ZodEnum<{
                prepare: "prepare";
                observe: "observe";
                publish: "publish";
                tag: "tag";
            }>>;
            status: z.ZodEnum<{
                unknown_registry: "unknown_registry";
                blocked_dependency_closure: "blocked_dependency_closure";
                blocked_invalid_artifact: "blocked_invalid_artifact";
                blocked_unknown_artifact: "blocked_unknown_artifact";
                blocked_unknown_dependency_closure: "blocked_unknown_dependency_closure";
                pending_eligible: "pending_eligible";
                pending_held: "pending_held";
                private_unpublishable: "private_unpublishable";
                published: "published";
            }>;
        }, z.core.$strict>>;
        schemaVersion: z.ZodLiteral<1>;
        summary: z.ZodObject<{
            blocked: z.ZodNumber;
            eligible: z.ZodNumber;
            held: z.ZodNumber;
            packages: z.ZodNumber;
            private: z.ZodNumber;
            published: z.ZodNumber;
            unknown: z.ZodNumber;
        }, z.core.$strict>;
    }, z.core.$strict>;
    preparedVersions: z.ZodNullable<z.ZodArray<z.ZodObject<{
        name: z.ZodString;
        version: z.ZodString;
    }, z.core.$strict>>>;
    selection: z.ZodArray<z.ZodString>;
}, z.core.$strict>;

// @public
export const createRepositoryScopeClassifiers: (snapshot: WorkspaceSnapshot, customClassifiers?: RepositoryScopeClassifiers, keepPrefix?: boolean) => RepositoryScopeClassifiers;

// @public
export type CurrentBranchKind = 'base' | 'changeset' | 'other' | 'release';

// @public
export const decideBranchAction: (state: BranchState, operation: BranchOperation) => BranchDecision;

// @public
export const deriveCommitFromBranch: (branch: string, options: {
    append?: string;
    scope: string;
}) => DerivedCommit | undefined;

// @public (undocumented)
export type DerivedCommit = {
    message: string;
    subject: string;
    type: BranchPrefix;
};

// @public
export function detectDefaultBranch(repoRoot: string): string;

// @public
export const executeBranchAction: (repoRoot: string, decision: BranchDecision, state: BranchState, options?: ExecuteBranchOptions) => void;

// @public
export type ExecuteBranchOptions = {
    inherit?: boolean;
    updateBase?: boolean;
};

// @public
export function executeReleasePublishPlan(plan: ReleasePublishPlan, options?: ExecuteReleasePublishPlanOptions): ReleasePublishResult;

// @public (undocumented)
export type ExecuteReleasePublishPlanOptions = {
    repoRoot?: string;
};

// @public
export function findNearestPackageJson(repoRoot: string, inputPath: string): null | string;

// @public
export const formatPullRequestPlan: (plan: PullRequestPlan) => string;

// @public
export function formatReleaseTagName(name: string, version: string): string;

// @public
export function formatScopes(scopes: ReadonlyArray<string>, format: ScopeFormat): string;

// @public
export const formatWorkflowPlan: (plan: WorkflowPlan) => string;

// @public
export const gatherBranchState: (repoRoot: string, options: {
    baseBranch: string;
    slug: string;
    targetPrefix: BranchPrefix;
}) => BranchState;

// @public
export const gatherWorkflowFacts: (repoRoot: string, options: {
    baseBranch: string;
}) => WorkflowFacts;

// @public
export const getBaseRelation: (repoRoot: string, baseBranch: string) => {
    aheadCount: number;
    behindCount: number;
    relation: BaseRelation;
};

// @public
export const getBaseSyncStatus: (repoRoot: string, baseBranch: string) => BaseSyncStatus;

// @public (undocumented)
export function getCurrentBranch(repoRoot: string): string;

// @public
export function getGitChangedFiles(options?: GitChangedFilesOptions): Array<string>;

// @public
export function getGitChangedFilesByArea(options?: GitChangedFilesOptions): Array<GitChangedFile>;

// @public
export function getPackageBinaryCommand(packageManager: PackageManager, binary: string, args?: ReadonlyArray<string>): PackageCommand;

// @public
export function getPackageManager(repoRoot: string, environment?: NodeJS.ProcessEnv): PackageManager;

// @public
export function getPackageScriptCommand(packageManager: PackageManager, script: string, args?: ReadonlyArray<string>): PackageCommand;

// @public
export function getRepoRoot(options?: {
    fallbackToCwd?: boolean;
}): string;

// @public
export const getWorkflowRemoteRelation: (repoRoot: string, branch: string) => WorkflowRemoteRelation;

// @public
export const getWorkingTreeStatus: (repoRoot: string) => WorkingTreeStatus;

// @public
export function getWorkspacePackagesList(filter?: (pkg: WorkspacePackage) => boolean, repoRoot?: string): Array<WorkspacePackage>;

// @public
export const getWorkspaceScopeClassifiers: (snapshot: WorkspaceSnapshot, keepPrefix?: boolean) => RepositoryScopeClassifiers;

// @public
export function getWorkspaceScopes(options: {
    keepPrefix?: boolean;
    overrides?: WorkspaceScopeOverrides;
    snapshot: WorkspaceSnapshot;
    standardScopes?: Readonly<Record<string, ReadonlyArray<string>>>;
}): ResolvedWorkspaceScopes;

// @public
export function getWorkspaceSnapshot(repoRoot?: string): WorkspaceSnapshot;

// @public
export type GitChangeArea = 'range' | 'staged' | 'unstaged' | 'untracked';

// @public
export type GitChangedFile = {
    areas: Array<GitChangeArea>;
    file: string;
};

// @public (undocumented)
export type GitChangedFilesOptions = {
    base?: string;
    cwd?: string;
    head?: string;
    includeStaged?: boolean;
    includeUnstaged?: boolean;
    includeUntracked?: boolean;
};

// @public
export const inferWorkflowIdentity: (branch: string) => undefined | WorkflowIdentity;

// @public
export const isWorkflowBranch: (branch: string) => boolean;

// @public
export const listWorkflowBranches: (repoRoot: string) => WorkflowBranches;

// @public
export function loadScopePathMatchers(repoRoot: string): Promise<WorkspaceScopeOverrides>;

// @public
export const localBranchExists: (repoRoot: string, branch: string) => boolean;

// @public
export function observeWorkspaceChangesetIntent(options?: ObserveWorkspaceChangesetIntentOptions): Promise<Array<WorkspaceChangesetIntent>>;

// @public (undocumented)
export type ObserveWorkspaceChangesetIntentOptions = {
    repoRoot?: string;
};

// @public
export function observeWorkspaceRegistry(options?: ObserveWorkspaceRegistryOptions): Array<WorkspaceRegistryObservation>;

// @public (undocumented)
export type ObserveWorkspaceRegistryOptions = {
    repoRoot?: string;
};

// @public (undocumented)
export type PackageCommand = {
    args: Array<string>;
    command: PackageManager;
};

// @public (undocumented)
export type PackageManager = 'npm' | 'pnpm';

// @public (undocumented)
export type PackageManagerResolution = {
    packageManager: PackageManager;
    source: 'default' | 'environment' | 'package.json';
};

// @public
export const parseBranchName: (branch: string) => ParsedBranchName | undefined;

// @public (undocumented)
export type ParsedBranchName = {
    prefix: BranchPrefix;
    slug: string;
};

// @public
export const planWorkflow: (facts: WorkflowFacts) => WorkflowPlan;

// @public
export const planWorkflowPullRequest: (base: string, identity: WorkflowIdentity, scope?: string) => PullRequestPlan;

// @public (undocumented)
export type PullRequestPlan = z.infer<typeof pullRequestPlanSchema>;

// @public (undocumented)
export type PullRequestPlanInput = {
    base: string;
    body?: string;
    head: string;
    title?: string;
};

// @public
export const pullRequestPlanSchema: z.ZodObject<{
    base: z.ZodString;
    body: z.ZodString;
    head: z.ZodString;
    title: z.ZodString;
}, z.core.$strict>;

// @public
export function readPackageName(packageJsonPath: string): null | string;

// @public
export const readWorkspaceEnvironment: (environment: Readonly<Record<string, unknown>>) => WorkspaceEnvironment;

// @public (undocumented)
export type ReleaseIntent = ReleasePackagePlan['intent'];

// @public
export type ReleasePackagePlan = ReleasePlan['packages'][number];

// @public (undocumented)
export type ReleasePlan = z.infer<typeof releasePlanSchema>;

// @public
export const releasePlanSchema: z.ZodObject<{
    execution: z.ZodObject<{
        operation: z.ZodLiteral<"observe">;
    }, z.core.$strict>;
    packages: z.ZodArray<z.ZodObject<{
        access: z.ZodOptional<z.ZodEnum<{
            public: "public";
            restricted: "restricted";
        }>>;
        doctor: z.ZodObject<{
            artifact: z.ZodEnum<{
                unknown: "unknown";
                invalid: "invalid";
                valid: "valid";
            }>;
            dependencyClosure: z.ZodEnum<{
                unknown: "unknown";
                valid: "valid";
                blocked: "blocked";
            }>;
        }, z.core.$strict>;
        gitTag: z.ZodDiscriminatedUnion<[z.ZodObject<{
            selected: z.ZodLiteral<false>;
        }, z.core.$strict>, z.ZodObject<{
            name: z.ZodString;
            selected: z.ZodLiteral<true>;
        }, z.core.$strict>], "selected">;
        intent: z.ZodDiscriminatedUnion<[z.ZodObject<{
            source: z.ZodLiteral<"none">;
        }, z.core.$strict>, z.ZodObject<{
            bump: z.ZodEnum<{
                major: "major";
                minor: "minor";
                patch: "patch";
            }>;
            reason: z.ZodString;
            source: z.ZodLiteral<"changesets">;
        }, z.core.$strict>, z.ZodObject<{
            bump: z.ZodEnum<{
                major: "major";
                minor: "minor";
                patch: "patch";
            }>;
            reason: z.ZodString;
            source: z.ZodLiteral<"explicit">;
        }, z.core.$strict>], "source">;
        name: z.ZodString;
        policy: z.ZodDiscriminatedUnion<[z.ZodObject<{
            channel: z.ZodString;
            decision: z.ZodLiteral<"selected">;
            reason: z.ZodString;
        }, z.core.$strict>, z.ZodObject<{
            decision: z.ZodLiteral<"held">;
            reason: z.ZodString;
        }, z.core.$strict>], "decision">;
        private: z.ZodBoolean;
        registry: z.ZodObject<{
            distTags: z.ZodRecord<z.ZodString, z.ZodString>;
            registryUrl: z.ZodNullable<z.ZodURL>;
            state: z.ZodEnum<{
                exists: "exists";
                missing: "missing";
                unknown_auth: "unknown_auth";
                unknown_network: "unknown_network";
                unknown_registry: "unknown_registry";
            }>;
        }, z.core.$strict>;
        version: z.ZodString;
        versionState: z.ZodDiscriminatedUnion<[z.ZodObject<{
            state: z.ZodLiteral<"current">;
        }, z.core.$strict>, z.ZodObject<{
            intendedVersion: z.ZodString;
            state: z.ZodLiteral<"pending">;
        }, z.core.$strict>], "state">;
        availableNextOperations: z.ZodArray<z.ZodEnum<{
            observe: "observe";
            prepare: "prepare";
            publish: "publish";
            tag: "tag";
        }>>;
        status: z.ZodEnum<{
            unknown_registry: "unknown_registry";
            blocked_dependency_closure: "blocked_dependency_closure";
            blocked_invalid_artifact: "blocked_invalid_artifact";
            blocked_unknown_artifact: "blocked_unknown_artifact";
            blocked_unknown_dependency_closure: "blocked_unknown_dependency_closure";
            pending_eligible: "pending_eligible";
            pending_held: "pending_held";
            private_unpublishable: "private_unpublishable";
            published: "published";
        }>;
    }, z.core.$strict>>;
    schemaVersion: z.ZodLiteral<1>;
    summary: z.ZodObject<{
        blocked: z.ZodNumber;
        eligible: z.ZodNumber;
        held: z.ZodNumber;
        packages: z.ZodNumber;
        private: z.ZodNumber;
        published: z.ZodNumber;
        unknown: z.ZodNumber;
    }, z.core.$strict>;
}, z.core.$strict>;

// @public (undocumented)
export type ReleasePreparationEvidence = z.infer<typeof releasePreparationEvidenceSchema>;

// @public
export const releasePreparationEvidenceSchema: z.ZodObject<{
    name: z.ZodString;
    version: z.ZodString;
}, z.core.$strict>;

// @public (undocumented)
export type ReleasePrepareBlocker = ReleasePreparePlan['blockers'][number];

// @public (undocumented)
export type ReleasePreparePackage = ReleasePreparePlan['packages'][number];

// @public (undocumented)
export type ReleasePreparePlan = z.infer<typeof releasePreparePlanSchema>;

// @public
export const releasePreparePlanSchema: z.ZodObject<{
    blockers: z.ZodArray<z.ZodDiscriminatedUnion<[z.ZodObject<{
        reason: z.ZodLiteral<"no_selected_packages">;
    }, z.core.$strict>, z.ZodObject<{
        reason: z.ZodLiteral<"no_preparable_packages">;
    }, z.core.$strict>, z.ZodObject<{
        names: z.ZodArray<z.ZodString>;
        reason: z.ZodLiteral<"unknown_selected_package">;
    }, z.core.$strict>, z.ZodObject<{
        reason: z.ZodLiteral<"working_tree_dirty">;
    }, z.core.$strict>, z.ZodObject<{
        reason: z.ZodLiteral<"working_tree_unknown">;
    }, z.core.$strict>], "reason">>;
    branch: z.ZodString;
    operation: z.ZodLiteral<"prepare">;
    packages: z.ZodArray<z.ZodDiscriminatedUnion<[z.ZodObject<{
        decision: z.ZodLiteral<"planned">;
        intendedVersion: z.ZodString;
        name: z.ZodString;
        private: z.ZodBoolean;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        decision: z.ZodLiteral<"blocked">;
        name: z.ZodString;
        private: z.ZodBoolean;
        reason: z.ZodLiteral<"no_pending_version">;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        decision: z.ZodLiteral<"not_selected">;
        name: z.ZodString;
        private: z.ZodBoolean;
        version: z.ZodString;
    }, z.core.$strict>], "decision">>;
    pullRequest: z.ZodNullable<z.ZodObject<{
        base: z.ZodString;
        body: z.ZodString;
        head: z.ZodString;
        title: z.ZodString;
    }, z.core.$strict>>;
    schemaVersion: z.ZodLiteral<1>;
    summary: z.ZodObject<{
        blocked: z.ZodNumber;
        packages: z.ZodNumber;
        planned: z.ZodNumber;
        private: z.ZodNumber;
        selected: z.ZodNumber;
    }, z.core.$strict>;
}, z.core.$strict>;

// @public (undocumented)
export type ReleasePreparePlanSummary = ReleasePreparePlan['summary'];

// @public (undocumented)
export type ReleasePublishArtifact = z.infer<typeof releasePublishArtifactSchema>;

// @public
export const releasePublishArtifactSchema: z.ZodObject<{
    integrity: z.ZodString;
    name: z.ZodString;
    tarball: z.ZodString;
    version: z.ZodString;
}, z.core.$strict>;

// @public (undocumented)
export type ReleasePublishAuthorization = ReleasePublishPlan['authorization'];

// @public (undocumented)
export type ReleasePublishCandidate = z.infer<typeof releasePublishCandidateSchema>;

// @public
export const releasePublishCandidateSchema: z.ZodObject<{
    artifact: z.ZodObject<{
        integrity: z.ZodString;
        name: z.ZodString;
        tarball: z.ZodString;
        version: z.ZodString;
    }, z.core.$strict>;
    doctor: z.ZodObject<{
        artifact: z.ZodEnum<{
            unknown: "unknown";
            invalid: "invalid";
            valid: "valid";
        }>;
        closure: z.ZodDiscriminatedUnion<[z.ZodObject<{
            edges: z.ZodArray<z.ZodDiscriminatedUnion<[z.ZodObject<{
                name: z.ZodString;
                range: z.ZodString;
                resolution: z.ZodLiteral<"available_in_registry">;
                satisfiedBy: z.ZodString;
            }, z.core.$strict>, z.ZodObject<{
                name: z.ZodString;
                range: z.ZodString;
                resolution: z.ZodLiteral<"included_in_cohort">;
            }, z.core.$strict>, z.ZodObject<{
                name: z.ZodString;
                resolution: z.ZodLiteral<"embedded_not_exposed">;
            }, z.core.$strict>, z.ZodObject<{
                name: z.ZodString;
                range: z.ZodString;
                resolution: z.ZodLiteral<"unavailable">;
            }, z.core.$strict>, z.ZodObject<{
                name: z.ZodString;
                resolution: z.ZodLiteral<"unknown">;
            }, z.core.$strict>], "resolution">>;
            state: z.ZodLiteral<"valid">;
        }, z.core.$strict>, z.ZodObject<{
            edges: z.ZodArray<z.ZodDiscriminatedUnion<[z.ZodObject<{
                name: z.ZodString;
                range: z.ZodString;
                resolution: z.ZodLiteral<"available_in_registry">;
                satisfiedBy: z.ZodString;
            }, z.core.$strict>, z.ZodObject<{
                name: z.ZodString;
                range: z.ZodString;
                resolution: z.ZodLiteral<"included_in_cohort">;
            }, z.core.$strict>, z.ZodObject<{
                name: z.ZodString;
                resolution: z.ZodLiteral<"embedded_not_exposed">;
            }, z.core.$strict>, z.ZodObject<{
                name: z.ZodString;
                range: z.ZodString;
                resolution: z.ZodLiteral<"unavailable">;
            }, z.core.$strict>, z.ZodObject<{
                name: z.ZodString;
                resolution: z.ZodLiteral<"unknown">;
            }, z.core.$strict>], "resolution">>;
            state: z.ZodLiteral<"blocked">;
        }, z.core.$strict>, z.ZodObject<{
            state: z.ZodLiteral<"unknown">;
        }, z.core.$strict>], "state">;
    }, z.core.$strict>;
    name: z.ZodString;
}, z.core.$strict>;

// @public (undocumented)
export type ReleasePublishClosureEdge = z.infer<typeof releasePublishClosureEdgeSchema>;

// @public
export const releasePublishClosureEdgeSchema: z.ZodDiscriminatedUnion<[z.ZodObject<{
    name: z.ZodString;
    range: z.ZodString;
    resolution: z.ZodLiteral<"available_in_registry">;
    satisfiedBy: z.ZodString;
}, z.core.$strict>, z.ZodObject<{
    name: z.ZodString;
    range: z.ZodString;
    resolution: z.ZodLiteral<"included_in_cohort">;
}, z.core.$strict>, z.ZodObject<{
    name: z.ZodString;
    resolution: z.ZodLiteral<"embedded_not_exposed">;
}, z.core.$strict>, z.ZodObject<{
    name: z.ZodString;
    range: z.ZodString;
    resolution: z.ZodLiteral<"unavailable">;
}, z.core.$strict>, z.ZodObject<{
    name: z.ZodString;
    resolution: z.ZodLiteral<"unknown">;
}, z.core.$strict>], "resolution">;

// @public (undocumented)
export type ReleasePublishDoctorEvidence = z.infer<typeof releasePublishDoctorEvidenceSchema>;

// @public
export const releasePublishDoctorEvidenceSchema: z.ZodObject<{
    artifact: z.ZodEnum<{
        unknown: "unknown";
        invalid: "invalid";
        valid: "valid";
    }>;
    closure: z.ZodDiscriminatedUnion<[z.ZodObject<{
        edges: z.ZodArray<z.ZodDiscriminatedUnion<[z.ZodObject<{
            name: z.ZodString;
            range: z.ZodString;
            resolution: z.ZodLiteral<"available_in_registry">;
            satisfiedBy: z.ZodString;
        }, z.core.$strict>, z.ZodObject<{
            name: z.ZodString;
            range: z.ZodString;
            resolution: z.ZodLiteral<"included_in_cohort">;
        }, z.core.$strict>, z.ZodObject<{
            name: z.ZodString;
            resolution: z.ZodLiteral<"embedded_not_exposed">;
        }, z.core.$strict>, z.ZodObject<{
            name: z.ZodString;
            range: z.ZodString;
            resolution: z.ZodLiteral<"unavailable">;
        }, z.core.$strict>, z.ZodObject<{
            name: z.ZodString;
            resolution: z.ZodLiteral<"unknown">;
        }, z.core.$strict>], "resolution">>;
        state: z.ZodLiteral<"valid">;
    }, z.core.$strict>, z.ZodObject<{
        edges: z.ZodArray<z.ZodDiscriminatedUnion<[z.ZodObject<{
            name: z.ZodString;
            range: z.ZodString;
            resolution: z.ZodLiteral<"available_in_registry">;
            satisfiedBy: z.ZodString;
        }, z.core.$strict>, z.ZodObject<{
            name: z.ZodString;
            range: z.ZodString;
            resolution: z.ZodLiteral<"included_in_cohort">;
        }, z.core.$strict>, z.ZodObject<{
            name: z.ZodString;
            resolution: z.ZodLiteral<"embedded_not_exposed">;
        }, z.core.$strict>, z.ZodObject<{
            name: z.ZodString;
            range: z.ZodString;
            resolution: z.ZodLiteral<"unavailable">;
        }, z.core.$strict>, z.ZodObject<{
            name: z.ZodString;
            resolution: z.ZodLiteral<"unknown">;
        }, z.core.$strict>], "resolution">>;
        state: z.ZodLiteral<"blocked">;
    }, z.core.$strict>, z.ZodObject<{
        state: z.ZodLiteral<"unknown">;
    }, z.core.$strict>], "state">;
}, z.core.$strict>;

// @public (undocumented)
export type ReleasePublishPackage = ReleasePublishPlan['packages'][number];

// @public (undocumented)
export type ReleasePublishPlan = z.infer<typeof releasePublishPlanSchema>;

// @public
export const releasePublishPlanSchema: z.ZodObject<{
    authorization: z.ZodDiscriminatedUnion<[z.ZodObject<{
        state: z.ZodLiteral<"authorized">;
    }, z.core.$strict>, z.ZodObject<{
        reasons: z.ZodArray<z.ZodEnum<{
            no_selected_packages: "no_selected_packages";
            no_publishable_packages: "no_publishable_packages";
            unresolved_prerequisites: "unresolved_prerequisites";
            unknown_selected_package: "unknown_selected_package";
        }>>;
        state: z.ZodLiteral<"withheld">;
    }, z.core.$strict>], "state">;
    channel: z.ZodString;
    operation: z.ZodLiteral<"publish">;
    packages: z.ZodArray<z.ZodDiscriminatedUnion<[z.ZodObject<{
        artifact: z.ZodObject<{
            integrity: z.ZodString;
            name: z.ZodString;
            tarball: z.ZodString;
            version: z.ZodString;
        }, z.core.$strict>;
        channel: z.ZodString;
        decision: z.ZodLiteral<"planned">;
        name: z.ZodString;
        registryUrl: z.ZodURL;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        decision: z.ZodLiteral<"already_published">;
        name: z.ZodString;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        decision: z.ZodLiteral<"not_selected">;
        name: z.ZodString;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        decision: z.ZodLiteral<"blocked_private">;
        name: z.ZodString;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        decision: z.ZodLiteral<"blocked_policy_held">;
        name: z.ZodString;
        reason: z.ZodString;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        decision: z.ZodLiteral<"blocked_registry_unknown">;
        name: z.ZodString;
        registryState: z.ZodEnum<{
            unknown_auth: "unknown_auth";
            unknown_network: "unknown_network";
            unknown_registry: "unknown_registry";
        }>;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        decision: z.ZodLiteral<"blocked_artifact_unavailable">;
        name: z.ZodString;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        decision: z.ZodLiteral<"blocked_artifact_invalid">;
        name: z.ZodString;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        decision: z.ZodLiteral<"unknown_artifact_facts">;
        name: z.ZodString;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        decision: z.ZodLiteral<"blocked_dependency_closure">;
        name: z.ZodString;
        unresolved: z.ZodArray<z.ZodString>;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        decision: z.ZodLiteral<"unknown_dependency_closure">;
        name: z.ZodString;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        decision: z.ZodLiteral<"blocked_closure_dependency_not_in_cohort">;
        missing: z.ZodArray<z.ZodString>;
        name: z.ZodString;
        version: z.ZodString;
    }, z.core.$strict>], "decision">>;
    schemaVersion: z.ZodLiteral<1>;
    summary: z.ZodObject<{
        alreadyPublished: z.ZodNumber;
        blocked: z.ZodNumber;
        packages: z.ZodNumber;
        planned: z.ZodNumber;
        selected: z.ZodNumber;
        unknown: z.ZodNumber;
    }, z.core.$strict>;
}, z.core.$strict>;

// @public (undocumented)
export type ReleasePublishPlanSummary = ReleasePublishPlan['summary'];

// @public (undocumented)
export type ReleasePublishResult = z.infer<typeof releasePublishResultSchema>;

// @public (undocumented)
export const releasePublishResultSchema: z.ZodObject<{
    operation: z.ZodLiteral<"publish">;
    schemaVersion: z.ZodLiteral<1>;
    started: z.ZodBoolean;
    steps: z.ZodArray<z.ZodDiscriminatedUnion<[z.ZodObject<{
        channel: z.ZodString;
        name: z.ZodString;
        outcome: z.ZodLiteral<"published">;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        name: z.ZodString;
        outcome: z.ZodLiteral<"skipped_already_published">;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        name: z.ZodString;
        observed: z.ZodString;
        outcome: z.ZodLiteral<"failed_registry_precheck">;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        expected: z.ZodString;
        name: z.ZodString;
        observed: z.ZodNullable<z.ZodString>;
        outcome: z.ZodLiteral<"failed_artifact_integrity">;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        detail: z.ZodString;
        name: z.ZodString;
        outcome: z.ZodLiteral<"failed_publish">;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        name: z.ZodString;
        observed: z.ZodString;
        outcome: z.ZodLiteral<"failed_verification">;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        channel: z.ZodString;
        detail: z.ZodString;
        name: z.ZodString;
        outcome: z.ZodLiteral<"failed_dist_tag">;
        version: z.ZodString;
    }, z.core.$strict>], "outcome">>;
    summary: z.ZodObject<{
        failed: z.ZodNumber;
        published: z.ZodNumber;
        skipped: z.ZodNumber;
    }, z.core.$strict>;
}, z.core.$strict>;

// @public (undocumented)
export type ReleasePublishStep = ReleasePublishResult['steps'][number];

// @public
export type ReleaseRegistryObservation = ReleasePackagePlan['registry'];

// @public (undocumented)
export type ReleaseTagBlocker = ReleaseTagPlan['blockers'][number];

// @public (undocumented)
export type ReleaseTagPackage = ReleaseTagPlan['packages'][number];

// @public (undocumented)
export type ReleaseTagPlan = z.infer<typeof releaseTagPlanSchema>;

// @public
export const releaseTagPlanSchema: z.ZodObject<{
    blockers: z.ZodArray<z.ZodDiscriminatedUnion<[z.ZodObject<{
        reason: z.ZodLiteral<"no_selected_packages">;
    }, z.core.$strict>, z.ZodObject<{
        reason: z.ZodLiteral<"no_taggable_packages">;
    }, z.core.$strict>, z.ZodObject<{
        names: z.ZodArray<z.ZodString>;
        reason: z.ZodLiteral<"unknown_selected_package">;
    }, z.core.$strict>, z.ZodObject<{
        reason: z.ZodLiteral<"tag_inventory_unavailable">;
    }, z.core.$strict>, z.ZodObject<{
        reason: z.ZodLiteral<"preparation_evidence_unavailable">;
    }, z.core.$strict>], "reason">>;
    operation: z.ZodLiteral<"tag">;
    packages: z.ZodArray<z.ZodDiscriminatedUnion<[z.ZodObject<{
        decision: z.ZodLiteral<"planned">;
        name: z.ZodString;
        private: z.ZodBoolean;
        tag: z.ZodString;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        decision: z.ZodLiteral<"blocked_preparation_incomplete">;
        intendedVersion: z.ZodString;
        name: z.ZodString;
        private: z.ZodBoolean;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        decision: z.ZodLiteral<"blocked_preparation_unproven">;
        name: z.ZodString;
        private: z.ZodBoolean;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        decision: z.ZodLiteral<"blocked_preparation_version_mismatch">;
        name: z.ZodString;
        preparedVersion: z.ZodString;
        private: z.ZodBoolean;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        decision: z.ZodLiteral<"unknown_preparation_evidence">;
        name: z.ZodString;
        private: z.ZodBoolean;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        decision: z.ZodLiteral<"blocked_already_tagged">;
        name: z.ZodString;
        private: z.ZodBoolean;
        tag: z.ZodString;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        decision: z.ZodLiteral<"unknown_tag_inventory">;
        name: z.ZodString;
        private: z.ZodBoolean;
        tag: z.ZodString;
        version: z.ZodString;
    }, z.core.$strict>, z.ZodObject<{
        decision: z.ZodLiteral<"not_selected">;
        name: z.ZodString;
        private: z.ZodBoolean;
        version: z.ZodString;
    }, z.core.$strict>], "decision">>;
    schemaVersion: z.ZodLiteral<1>;
    summary: z.ZodObject<{
        blocked: z.ZodNumber;
        packages: z.ZodNumber;
        planned: z.ZodNumber;
        private: z.ZodNumber;
        selected: z.ZodNumber;
        unknown: z.ZodNumber;
    }, z.core.$strict>;
}, z.core.$strict>;

// @public (undocumented)
export type ReleaseTagPlanSummary = ReleaseTagPlan['summary'];

// @public (undocumented)
export type ReleaseVersionState = ReleasePackagePlan['versionState'];

// @public
export const remoteBranchExists: (repoRoot: string, branch: string) => boolean;

// @public
export const renderGhPrCreateCommand: (plan: PullRequestPlan) => string;

// @public
export function renderReleasePlanMarkdown(plan: ReleasePlan): string;

// @public
export function renderReleasePlanTerminal(plan: ReleasePlan): string;

// @public (undocumented)
export type RepositoryScopeClassifiers = StringClassifiers;

// @public (undocumented)
export type RepositoryScopeResolution = {
    matches: Readonly<Record<string, Array<string>>>;
    scopes: Array<string>;
    unmatched: Array<string>;
};

// @public
export function resolveBaseBranch(repoRoot: string, ...preferred: ReadonlyArray<string | undefined>): string;

// @public
export type ResolvedWorkspaceScopes = {
    classifiers: RepositoryScopeClassifiers;
    definitions: WorkspaceScopes;
    names: Array<string>;
    sources: Readonly<Record<string, Array<WorkspaceScopeSource>>>;
};

// @public
export function resolvePackageManager(repoRoot: string, environment?: NodeJS.ProcessEnv): PackageManagerResolution;

// @public
export const resolveRepositoryScopes: (files: ReadonlyArray<string>, classifiers: RepositoryScopeClassifiers) => RepositoryScopeResolution;

// @public (undocumented)
export function runCommitMessageHook(repoRoot: string, messageFile: string): void;

// @public (undocumented)
export function runLintStaged(repoRoot: string, environment?: NodeJS.ProcessEnv): void;

// @public (undocumented)
export function runPackageBinary(repoRoot: string, binary: string, args?: ReadonlyArray<string>, options?: RunCommandOptions): CommandResult;

// @public
export function runPackageManager(repoRoot: string, args?: ReadonlyArray<string>, options?: RunCommandOptions): CommandResult;

// @public (undocumented)
export function runPackageScript(repoRoot: string, script: string, args?: ReadonlyArray<string>, options?: RunCommandOptions): CommandResult;

// @public (undocumented)
export function runSnailSh(repoRoot: string, command: string, args?: ReadonlyArray<string>): void;

// @public
export function safeGetWorkspaceSnapshot(repoRoot?: string): WorkspaceSnapshotResult;

// @public (undocumented)
export type ScopeFormat = 'csv' | 'list';

// @public
export function shortenScopeName(scopeName: string, keepPrefix?: boolean): string;

// @public (undocumented)
export function showHookSection(repoRoot: string, hookName: string): void;

// @public
export const SNAILICID3_COMMITLINT_CONFIG_KEY: "snailicid3";

// @public
export const STANDARD_WORKSPACE_SCOPES: {
    readonly actions: readonly [".github/workflows/**", ".github/actions/**", ".github/scripts/**"];
    readonly notes: readonly ["notes/**"];
    readonly scripts: readonly ["scripts/**"];
};

// @public
export const switchBranch: (repoRoot: string, branch: string, options?: {
    inherit?: boolean;
}) => void;

// @public (undocumented)
export function validateBranchName(repoRoot: string): void;

// @public (undocumented)
export type WorkflowActionId = 'commit' | 'push' | 'stage' | 'start-changeset' | 'switch-workflow-branch' | 'update-base';

// @public (undocumented)
export type WorkflowBranches = {
    local: Array<string>;
    remote: Array<string>;
};

// @public
export type WorkflowFacts = {
    aheadCount: number;
    baseBranch: string;
    baseRelation: BaseRelation;
    behindCount: number;
    currentBranch: string;
    detached: boolean;
    identity: undefined | WorkflowIdentity;
    matchingBranches: WorkflowBranches;
    remoteRelation: WorkflowRemoteRelation;
    stagedFiles: Array<string>;
    workflowBranches: WorkflowBranches;
    workingTree: 'clean' | 'dirty';
};

// @public
export type WorkflowIdentity = {
    branch: string;
    mode: BranchPrefix;
    slug: string;
};

// @public
export type WorkflowNextAction = {
    command: string;
    consequence: string;
    id: WorkflowActionId;
};

// @public (undocumented)
export type WorkflowPlan = {
    facts: WorkflowFacts;
    nextActions: Array<WorkflowNextAction>;
    stacked: boolean;
    unavailableReason: string | undefined;
    warnings: Array<string>;
};

// @public (undocumented)
export type WorkflowRemoteRelation = {
    aheadCount: number;
    behindCount: number;
    relation: BaseRelation;
};

// @public (undocumented)
export type WorkingTreeStatus = 'clean' | 'dirty';

// @public
export type WorkspaceChangesetIntent = {
    intent: ReleaseIntent;
    name: string;
    version: string;
    versionState: ReleaseVersionState;
};

// @public (undocumented)
export type WorkspaceEnvironment = z.output<typeof workspaceEnvironment.schema>;

// @public
export const workspaceEnvironment: EnvironmentDefinition<    {
readonly allowDirty: z.ZodType<boolean, unknown, z.core.$ZodTypeInternals<boolean, unknown>>;
readonly baseBranch: z.ZodDefault<z.ZodString>;
readonly commandName: z.ZodDefault<z.ZodString>;
readonly logging: z.ZodType<boolean, unknown, z.core.$ZodTypeInternals<boolean, unknown>>;
readonly packageManagerOverride: z.ZodOptional<z.ZodEnum<{
npm: "npm";
pnpm: "pnpm";
}>>;
readonly patchCwd: z.ZodDefault<z.ZodString>;
readonly prefix: z.ZodDefault<z.ZodString>;
readonly prefixOverride: z.ZodDefault<z.ZodString>;
readonly protectedBranches: z.ZodType<string[], unknown, z.core.$ZodTypeInternals<string[], unknown>>;
readonly skipCommitlint: z.ZodType<boolean, unknown, z.core.$ZodTypeInternals<boolean, unknown>>;
readonly skipLintStaged: z.ZodType<boolean, unknown, z.core.$ZodTypeInternals<boolean, unknown>>;
}>;

// @public (undocumented)
export type WorkspacePackage = {
    name: string;
    path: string;
    private?: boolean;
    version: string;
};

// @public
export const workspacePackageManagerOutputSchema: (repoRoot: string) => z.ZodType<Array<WorkspacePackage>, string>;

// @public
export const workspacePackageRecordSchema: (repoRoot: string) => z.ZodType<WorkspacePackage>;

// @public
export type WorkspaceRegistryObservation = {
    name: string;
    registry: ReleaseRegistryObservation;
    version: string;
};

// @public
export type WorkspaceScopeDefinition = ReadonlyArray<string> | true;

// @public
export type WorkspaceScopeOverride = boolean | ReadonlyArray<string> | undefined;

// @public (undocumented)
export type WorkspaceScopeOverrides = Readonly<Record<string, WorkspaceScopeOverride>>;

// @public
export const workspaceScopeOverridesSchema: z.ZodRecord<z.ZodString, z.ZodOptional<z.ZodUnion<readonly [z.ZodBoolean, z.ZodArray<z.ZodString>]>>>;

// @public (undocumented)
export type WorkspaceScopes = Readonly<Record<string, WorkspaceScopeDefinition>>;

// @public
export type WorkspaceScopeSource = 'override' | 'package' | 'standard';

// @public
export type WorkspaceSnapshot = {
    list: Array<WorkspacePackage>;
    lookup: Map<string, WorkspacePackage>;
    repoRoot: string;
};

// @public (undocumented)
export type WorkspaceSnapshotResult = {
    data: WorkspaceSnapshot;
    success: true;
} | {
    error: string;
    success: false;
};

// (No @packageDocumentation comment for this package)

```
