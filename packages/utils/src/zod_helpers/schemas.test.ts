...
describe('Zod helpers', () => {
    test('ENSURE ARRAY', () => {
        ...
        expect(getArrRecursiveSchemaTest.parse(recursiveTest)).toStrictEqual(recursiveTest)
        expect(getArrRecursiveSchemaTest.parse(['gill', 'tunney'])).toStrictEqual(endResult)
        expect(getArrRecursiveSchemaTest.parse([['gill'], 'tunney'])).toStrictEqual(endResult)
        expect(getArrRecursiveSchemaTest.parse(endResult)).toStrictEqual(endResult)
        ...
        //inputs
        const __inputInA: z.input<typeof getArrRecursiveSchemaTest> = ['33', 'gillian']
        ...
        const __inputOutB: z.output<typeof getArrRecursiveSchemaTest> = [['33'], ['gillian']]

        const __inputOutC: z.input<typeof getArrRecursiveSchemaTest> = 'gillian'
        const __inputOutD: z.input<typeof getArrRecursiveSchemaTest> = ['gillian']

        const outputOutD2: z.output<typeof getArrRecursiveSchemaTest> = [['gillian']]
    ...
    test('resolveRegExpSchema', () => {
        ...
        const getREGEXPSchemaTest = ensureArray<typeof testschema>(testschema).transform(
            (value) => {
        ...
        expect(getREGEXPSchemaTest.parse([/^0x/, 'FFFF00']).test('440x')).toBe(false)
        expect(getREGEXPSchemaTest.parse([/^0x/, 'FFFF00']).test('0x3476')).toBe(true)
        expect(getREGEXPSchemaTest.parse(['^0x', 'FFFF00']).test('0x3476')).toBe(true)
        expect(getREGEXPSchemaTest.parse(['^0x', 'FFFF00']).test('FFFF00333')).toBe(true)
        expect(getREGEXPSchemaTest.parse(['^0x', 'FFFF00']).test('34760x')).toBe(false)

        //this is because the ^ is escaped to a literal  rather than the regexp to come first !!
        expect(getREGEXPSchemaTest.parse(['^+0x', 'FFFF00']).test('^+0x234760x')).toBe(true)
