...
import { isBigInt, isNumber, isString } from '../typeguard/utility.typeguards.js'
/**
...
/** @group Parse */
export const isParsableToNumeric = <Type extends PossibleNumeric>(value: Type): value is Type => {
    return isPossibleNumeric(value, false)
}

/** @group Parse */
export const parseToNumeric = <Type extends PossibleNumeric>(value: Type): Numeric | undefined => {
    //|| isNumber(value)
 ...
 * @group Parse
 */
export const parseStringToNumeric = <Type extends string>(value: Type): Numeric | undefined => {
    return toStringNumeric(value, false)
}

/** @group Parse */
export const parseStringToInteger = <Type extends string>(value: Type): number | undefined => {
    if (isStringNumeric<Type>(value, false)) {
        const result = toStringNumeric(value, false)
        return result !== undefined && isNumber<number>(result) ? Math.round(result) : undefined
    }
    return undefined
}
