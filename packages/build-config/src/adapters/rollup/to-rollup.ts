 ...
 *
 * One `RollupOptions` object is generated per entry. Each object may have multiple outputs (one per `outputKind`).
 *
...
export function toRollupConfig(
    ...
    return plan.entries.map((entry) => {
        ...
        if (entry.minify) {
            ...
            const minOutputs: Array<OutputOptions> = entry.outputKinds.map(
                (kind) => {
