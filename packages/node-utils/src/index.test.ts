...
describe('encodeBase64 getImageBase64', () => {
    test('returns an encoded string for a file path', () => {
        const imageTest = getImageBase64('./sample_image/testimage.jpeg', 'jpeg')
        expect(imageTest.length).toBeGreaterThan(20)
        const imageTest2 = getImageBase64('./sample_image/_badurltestimage.jpeg', 'jpeg')
...
describe('getFilePathArr getFilePathObj', () => {
    test('returns filepath object', () => {
        ...
        const filePathObj2 = getFilePathObj('./sample_image/_badurltestimage.jpeg')
    ...
    test('returns allowed extension types', () => {
        ...
        expect(getExistingPathType('./sample_image/*.{png,jpg}')).toBe(undefined)
        expect(getExistingPathType('./sample_image/kitten2.jpeg')).toBe('file')
        expect(getExistingPathType('./sample_image/kitten332.jpeg')).toBe(undefined)

        expect(isFile('./sample_image//')).toBe(false)
        expect(isFile('./sample_image/fileme.png', 'jpeg')).toBe(false)
        expect(isFile('./sample_image/fileme.png', ['jpeg', 'png', 'svg'])).toBe(true)
